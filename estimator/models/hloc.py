...
class ReconstructionError(Exception):
	"""Custom exception for reconstruction failures"""
	pass

def timeout_handler(signum, frame):
	"""Handler for SIGABRT and other signals"""
	raise ReconstructionError("Reconstruction process was aborted (likely COLMAP crash)")
...
class HlocEstimator(BaseEstimator):
	...
	def estimate_pose(self, scene_root, references, query, cam_opts=None, mapper_opts=None):
		...
		##### Reconstruction
		try:
			# Set up signal handler to catch COLMAP crashes
			old_sigabrt_handler = signal.signal(signal.SIGABRT, timeout_handler)
			try:
				...
				signal.signal(signal.SIGABRT, old_sigabrt_handler)
		except (RuntimeError, SystemError, ReconstructionError, Exception) as e:
			print(f"Reconstruction failed with error: {e}")
			print("Skipping this reconstruction and continuing...")
			return (None,)*5
		
		if not model or model.num_points3D() < 100: 
			print(f"Reconstruction failed: model={model}, points3D={model.num_points3D() if model else 0}")
			return (None,)*5
	...
	def _forward(self, scene_root, refs, query, ref_poses, ref_ints, query_int, est_opts):
		...
		try:
			...
			if est_opts['known_extrinsics']:
				...
				if model is None: return (None,)*3
				self.model = model
			...
			self.ret = result['ret']
			self.log = result['log']
			return (focal, np.linalg.inv(pose), loss)

		except Exception as e:
			print(f"Exception occurred in _forward: {e}")
			return (None,)*3
